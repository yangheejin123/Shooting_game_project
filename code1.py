...
pygame.display.set_caption("My Game") #이름 설정
clock = pygame.time.Clock() #화면을 초당 몇번 출력하는가
...
####### 플레이어 1, 2
player1_img = pygame.image.load("player1.png") #이미지 로드라는 함수를 이용해야지 변수에 저장이 되나봄
...
background_rect = background_img.get_rect() #이건 뭐지
...
while running:
    if menu:
        while (1):
            if main_menu() == 2: # 게임 시작 선택
                ...
                if game_choose()==5: #개인모드
                    break
                elif game_choose() == 6: #메인메뉴
                    break
                elif game_choose()==1:
                    pass
    ...
    screen.blit(background_img, background_rect) #이미지 출력?
    ...
    pygame.display.flip() #항상 파이게임의 메인루프 끝에 사용해주어야함! 화면을 업데이트하는 함수
